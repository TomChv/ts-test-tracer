...
const REPO_PKG_LOCATION = "packages/mocha-test";
const MOD_PKG_LOCATION = `/src/${REPO_PKG_LOCATION}`;
const REPO_EXAMPLE_LOCATION = "examples/mocha";
...
export class MochaTest {
  ...
  constructor(
    @argument({
      ...
      ignore: [
        "*",
        "!packages/mocha-test",
        "packages/mocha-test/node_modules",
        "packages/mocha-test/dist",
        "!examples/mocha",
  ...
  @func()
  withUpdatedLockefile(): MochaTest {
    this.source = this.devContainer()
  ...
  @func()
  async withBumpedVersion(version: string): Promise<MochaTest> {
    this.source = this.devContainer()
